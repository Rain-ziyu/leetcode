...
//
//
// Related Topics 贪心 数组 动态规划 排序 👍 31 👎 0
...
func minimumCost(m int, n int, horizontalCut []int, verticalCut []int) int {
	...
	sort.Slice(horizontalCut, func(i, j int) bool {
		return horizontalCut[j]-horizontalCut[i] < 0
	})
	sort.Slice(verticalCut, func(i, j int) bool {
		return verticalCut[j]-verticalCut[i] < 0
	})
	res := 0
	for x+y != m+n {
		fmt.Println(res, x, y, horizontalCut, verticalCut)
		if len(verticalCut) == 0 || (len(horizontalCut) != 0 && horizontalCut[0] > verticalCut[0]) {
			res += horizontalCut[0] * y
			x++
