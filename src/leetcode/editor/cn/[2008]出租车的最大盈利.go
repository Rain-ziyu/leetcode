...
// 
//
// Related Topics 数组 二分查找 动态规划 排序 👍 152 👎 0
package  main
...
func maxTaxiEarnings(n int, rides [][]int) int64 {
	...
	for i := 1; i < len(rides)+1; i++ {
		//找到 一个离当前乘车上车位置最近的下车乘客
		search := sort.Search(i, func(x int) bool {
			return rides[x][1] > rides[i-1][0]
		})
	//	取
	dp[i] = max(dp[i-1],dp[search]+int64(rides[i-1][1]-rides[i-1][0]+rides[i-1][2]))
	}
